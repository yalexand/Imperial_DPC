%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DPC_TV solves the phase and amplitude with L2+TV regularization         %
%                                                                         %
% Inputs:                                                                 %
...
%        reg_L2       : parameters for L2 regularization                  %
% Outputs:                                                                %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [amplitude, phase] = DPC_TV(zernike_coeff, rho, z_k, u_k, reg_L2)
    ...
    M11         = sum(abs(Hr).^2, 3) + rho * abs(Dx).^2 + rho * abs(Dy).^2 + reg_L2(1);
    M12         = sum(conj(Hr).*Hi, 3);
    M21         = sum(conj(Hi).*Hr, 3);
    M22         = sum(abs(Hi).^2, 3) + rho * abs(Dx).^2 + rho * abs(Dy).^2 + reg_L2(2);
